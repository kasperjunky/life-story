// Version 4.3 Final - Adding Retry Mechanism for Encouraging Rewrite
...
// Fetch insights from ChatGPT with proper prompts
    ...
    const response = await fetch("https://interactive-backend.onrender.com/api/generate", {
      ...
      body: JSON.stringify({
        ...
        prompts: {
          ...
          practicalAdvice: "Provide 3-5 specific, actionable steps the user can take to improve their life and perception. The response must begin exactly with: 'Here are a few practical ways to keep on empowering your inner story:' followed by concrete examples. Use only second-person language."
        }
      })
    });
...
// Retry mechanism for Encouraging Rewrite (Agent 2)
async function fetchEncouragingRewrite(retryCount = 0) {
  const maxRetries = 3;
  const waitTime = 10000; // 10 seconds
  const rewriteInsights = await getInsightsFromChatGPT(selectedAnswers, false, "Encouraging Rewrite");
  let rewriteText = rewriteInsights?.encouragingRewrite ? rewriteInsights.encouragingRewrite.trim() : "";
  // Check if rewriteText is valid and not just a header
  if (!rewriteText || rewriteText.toLowerCase().includes("life story rewrite")) {
    if (retryCount < maxRetries) {
      console.log(`Retrying Encouraging Rewrite... Attempt ${retryCount + 1}`);
      await new Promise(resolve => setTimeout(resolve, waitTime));
      return fetchEncouragingRewrite(retryCount + 1);
    } else {
      return null;
    }
  }
...
async function showResults() {
  ...
  // Agent 2: Encouraging Rewrite with Retry
  setTimeout(async () => {
    encouragingRewriteHeading.style.display = "block";
    loadingContainer.style.display = "block";
    const rewriteText = await fetchEncouragingRewrite();
    loadingContainer.style.display = "none";
    if (rewriteText) {
      let processedText = rewriteText;
      processedText = processedText.replace(/you is/gi, "you are")
                                   .replace(/This user|The user/gi, "you")
                                   .replace(/they/gi, "you");
      encouragingRewrite.textContent = processedText;
    } else {
...
adviceButton.addEventListener("click", async () => {
  ...
  if (insights?.practicalAdvice && insights.practicalAdvice.length > 0) {
    ...
    const formattedAdvice = adviceList.map((advice, index) => {
      let processed = advice.replace(/This individual has/gi, "You have")
                            ...
                            .replace(/they/gi, "you")
                            .trim();
      // Remove unwanted introductory phrases from practical advice
  ...
  // Set up PDF download content after practical advice is loaded
