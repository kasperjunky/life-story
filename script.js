...
async function showResults() {
  ...
  // 🔹 FIX: Ensure insights are properly received
  ...
  // ✅ FIX: Ensure the Encouraging Rewrite Insight is displayed properly
  if (insights.encouragingRewrite && insights.encouragingRewrite.trim() !== "") {
    ...
    encouragingRewrite.textContent = currentLanguage === "en"
      ? "No rewritten version available."
      : "אין גרסה מחודשת זמינה.";
  }

  // Update the heading for Encouraging Rewrite dynamically
  ...
  if (encouragingRewriteHeading) {
    encouragingRewriteHeading.textContent =
      currentLanguage === "en"
        ? "Let's reframe your story in an empowering way."
        : "בואו ננסח מחדש את הסיפור שלכם בדרך מעצימה.";
  }
