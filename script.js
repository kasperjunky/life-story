// Version 3.5 - Enforcing a Specific Starting Phrase for Insight Agent 1 and Refining Other Agents
    ...
    try {
        ...
        const response = await fetch("https://interactive-backend.onrender.com/api/generate", {
            ...
            body: JSON.stringify({
                ...
                includeAdvice,
                prompts: {
                    storySummary: "Summarize the user's answers objectively without any encouragement. Use only second-person ('you', 'your') language. The response must start exactly with: 'Based on your answers, it seems like your life' followed by a clear, factual summary.",
                    ...
                    practicalAdvice: "Provide 3-5 specific, actionable steps the user can take to improve their life and perception of it. The advice must be truly practical (e.g., 'Call an old friend' instead of vague suggestions). The first sentence must not be a bullet point."
                }
            })
        });
...
async function showResults() {
    ...
    if (insights?.storySummary) {
        ...
        // Define the exact required prefix:
        const prefix = "Based on your answers, it seems like your life";
        // Remove any duplicate occurrences of the prefix
        summary = summary.replace(new RegExp("^(" + prefix + ")+", "i"), "");
        // Prepend the prefix if not present
