// Version 2.6 - Refining Insight Agent 1 (Your Life Story)
    ...
    try {
        ...
        const response = await fetch("https://interactive-backend.onrender.com/api/generate", {
            ...
            body: JSON.stringify({ 
                ...
                includeAdvice,
                prompts: {
                    storySummary: "Analyze the user's responses and generate a narrative summary about their life. The response **must always start with 'Based on your answers...'** and use second-person ('you', 'your'). Avoid phrases like 'the user' or 'this individual'. Ensure a direct, engaging tone.",
...
async function showResults() {
    ...
    if (insights?.storySummary) {
        storySummary.textContent = insights.storySummary
            .replace(/^Based on the questionnaire answers you provided,?/, "Based on your answers,") // ✅ Force correct phrasing
            .replace(/\bthe user\b|\bthis individual\b/g, "you"); // ✅ Ensure second-person usage
    }
