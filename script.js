...
async function showResults() {
  ...
  practicalAdvice.innerHTML = ""; // Clear any previous advice
}
...
adviceButton.addEventListener("click", async () => {
  ...
  if (insights.practicalAdvice) {
    practicalAdvice.innerHTML = `
      <p>$
        {currentLanguage === "en"
          ? "To further empower your story and improve your life, here are some practical suggestions:"
          : "כדי לחזק את סיפור חייך ולשפר את חייך, הנה כמה הצעות מעשיות:"}
      </p>
      <ul>${insights.practicalAdvice.map((advice) => `<li>${advice}</li>`).join("")}</ul>
    `;
  } else {
    practicalAdvice.innerHTML = currentLanguage === "en"
