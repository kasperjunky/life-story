// Version 4.2.1 Final - Optimized Agent 3 Practical Advice and Enhanced PDF Layout
  ...
  if (insights?.practicalAdvice && insights.practicalAdvice.length > 0) {
    ...
    const formattedAdvice = adviceList.map((advice, index) => {
      let processed = advice.replace(/This individual has/gi, "You have")
                            ...
                            .replace(/they/gi, "you")
                            .trim();
      // For the first item, enforce the desired introduction and remove extra text before it
      if (index === 0) {
        const lowerProcessed = processed.toLowerCase();
        const lowerDesired = desiredIntro.toLowerCase();
        const pos = lowerProcessed.indexOf(lowerDesired);
        if (pos !== -1) {
          processed = processed.substring(pos);
        } else {
