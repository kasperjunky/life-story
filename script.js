// Version 3.5 - Enforcing a Single Starting Phrase for Insight Agent 1 & Refining Other Agents
  ...
  try {
    ...
    const response = await fetch("https://interactive-backend.onrender.com/api/generate", {
      ...
      body: JSON.stringify({
        ...
        includeAdvice,
        prompts: {
          storySummary: "Summarize the user's answers objectively without added encouragement. Use only second-person language. The response must start exactly with: 'Based on your answers, it seems like your life' followed by a clear, factual summary.",
          encouragingRewrite: "Rewrite the user's life story with a supportive, empowering tone. Use only second-person language. Begin with: 'Here's a new way to view your story in an empowering light: You navigate...' and provide an uplifting yet realistic perspective.",
          practicalAdvice: "Provide 3-5 specific, actionable steps the user can take to improve their life and perception. The advice must be truly practical (e.g., 'Call an old friend' instead of vague suggestions). The first sentence must not be a bullet point."
        }
      })
    });
...
async function showResults() {
  ...
  if (insights?.storySummary) {
    ...
    const prefix = "Based on your answers, it seems like your life";
    // Remove any duplicate occurrences of the prefix after the first one
    if (summary.toLowerCase().startsWith(prefix.toLowerCase())) {
      // Remove subsequent occurrences of the prefix
      summary = prefix + summary.substring(prefix.length).replace(new RegExp(prefix, "gi"), "").trim();
    } else {
...
adviceButton.addEventListener("click", async () => {
  ...
  if (insights?.practicalAdvice && insights.practicalAdvice.length > 0) {
    ...
    const formattedAdvice = adviceList.map((advice, index) => {
      if (index === 0) {
        // Ensure the first advice is personal and not preceded by any bullet or number
        return `<strong>${advice.replace(/This individual has/gi, "You have").replace(/^[•\d\.\s]*/, "").trim()}</strong>`;
      } else {
        // Format subsequent advice as list items without duplicate numbering or bullet markers
