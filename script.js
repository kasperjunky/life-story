// Version 3.6 - Enforcing a Clean Starting Phrase for Insight Agent 1 & Refining Other Agents
...
async function showResults() {
  ...
  if (insights?.storySummary) {
    ...
    // Remove unwanted phrases and fix grammar
    ...
    // Enforce the desired starting phrase:
    const desiredPrefix = "Based on your answers, it seems like your life";
    // If the summary contains any prefix already, remove everything up to the last occurrence of it.
    const lowerSummary = summary.toLowerCase();
    const lowerPrefix = desiredPrefix.toLowerCase();
    if (lowerSummary.includes(lowerPrefix)) {
      summary = summary.substring(lowerSummary.lastIndexOf(lowerPrefix) + lowerPrefix.length).trim();
    }
