// Version 4.2 Final - Refined Practical Advice for Agent 3 and Enhanced PDF Layout
...
// Warm-Up API Function (to pre-warm backend)
  ...
  if (insights?.practicalAdvice && insights.practicalAdvice.length > 0) {
    ...
    const formattedAdvice = adviceList.map((advice, index) => {
      let processed = advice.replace(/This individual has/gi, "You have")
                            ...
                            .replace(/they/gi, "you")
                            .trim();
      // Remove unwanted introductory phrase from Agent 3 output
      processed = processed.replace(/^To improve(?: their current state)?(?:,?\s*this user can take practical steps such as:)?\s*/i, "");
      if (index === 0) {
        // Ensure the first item starts with the desired introduction if not already present
