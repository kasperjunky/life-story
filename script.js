// Version 3.7 - Refining Insight Agent 1 Summary and Fixing Pronoun Issues
  ...
  try {
    ...
    const response = await fetch("https://interactive-backend.onrender.com/api/generate", {
      ...
      body: JSON.stringify({
        ...
        includeAdvice,
        prompts: {
          storySummary: "Summarize the user's answers objectively without added encouragement. Use only second-person language. The response must start exactly with: 'Based on your answers, you are a person who values' followed by a clear, factual summary of their responses.",
...
async function showResults() {
  ...
  if (insights?.storySummary) {
    ...
    // Remove unwanted phrases and fix grammar errors:
    ...
    summary = summary.replace(/unsure of themselves/gi, "unsure of yourself");
    summary = summary.trim();
    // Enforce the desired starting phrase exactly:
    const desiredPrefix = "Based on your answers, you are a person who values";
    // Remove any occurrence of the prefix from the start
    summary = summary.replace(new RegExp("^(?:" + desiredPrefix.replace(/[-\/\\^$*+?.()|[\]{}]/g, '\\$&') + "\\s*)+", "i"), "");
    // Prepend the desired prefix
    summary = desiredPrefix + " " + summary;
