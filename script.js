// Version 3.8 - Including API Warm-Up on Page Load & Previous Functionality
...
// 🟢 SECTION 2: Warm-Up API Function
async function warmUpAPI() {
  try {
    // Adjust the URL to point to your keep-alive endpoint.
...
// 🟢 SECTION 3: Fetch Insights from ChatGPT
...
// 🟢 SECTION 5: Handle Answer Selection
...
// 🟢 SECTION 6: Move to Next Question
...
// 🟢 SECTION 7: Show Results (Step-by-Step Insights)
async function showResults() {
  ...
  setTimeout(async () => {
    ...
    if (rewriteInsights?.encouragingRewrite) {
      ...
      // Remove any unwanted introductory phrase and fix pronoun errors
...
// 🟢 SECTION 8: Fetch Practical Advice & Fix Formatting
adviceButton.addEventListener("click", async () => {
  ...
  if (insights?.practicalAdvice && insights.practicalAdvice.length > 0) {
    // Limit to a maximum of 5 items (at least 3 if available)
    ...
    const formattedAdvice = adviceList.map((advice, index) => {
      let processed = advice.replace(/This individual has/gi, "You have")
                            ...
                            .trim();
      if (index === 0) {
        // The first piece should not have a bullet or number
...
// 🟢 SECTION 9: Ensure PDF Download Works
...
// 🟢 SECTION 10: Start Quiz on Page Load & Warm-Up API
