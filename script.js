// Version 2.9 - Fixing Insight Formatting & Practical Advice List Issues
...
adviceButton.addEventListener("click", async () => {
    ...
    practicalAdvice.innerHTML = insights?.practicalAdvice?.length > 0
        ? insights.practicalAdvice.map((advice, index) => 
            index === 0 
                ? `<strong>${advice}</strong>` // ✅ Ensuring no bullet for first sentence
                : `<li>${advice.replace(/^(\d+\.\s*)/, "")}</li>` // ✅ Removing extra numbering
        ).join("")
