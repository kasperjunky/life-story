...
const adviceCheckbox = document.getElementById("advice-checkbox");
...
async function getInsightsFromChatGPT(answers) {
  const includeAdvice = adviceCheckbox.checked; // Check if the user wants practical advice
  ...
  try {
    const response = await fetch("https://interactive-backend.onrender.com/api/generate", {
      ...
      body: JSON.stringify({
        ...
        answers,
        includeAdvice // Send this flag to the backend
      })
    });
