// Version 3.0 - Fixing Insight Formatting, Alignment, and PDF Download
    ...
    try {
        ...
        const response = await fetch("https://interactive-backend.onrender.com/api/generate", {
            ...
            body: JSON.stringify({ 
                ...
                includeAdvice,
                prompts: {
                    storySummary: "Summarize the user's answers objectively, without encouragement or empowerment. The response **must always use second-person ('you', 'your')** instead of third-person ('the user', 'they'). Start with: 'Based on your answers...' and ensure a neutral, factual summary.",
...
// 🟢 SECTION 7: Fetch Practical Advice & Fix Bullet Alignment
adviceButton.addEventListener("click", async () => {
    ...
    practicalAdvice.innerHTML = insights?.practicalAdvice?.length > 0
        ? insights.practicalAdvice.map((advice, index) => 
            index === 0 
                ? `<strong>${advice.replace("This individual has", "You have")}</strong>` // ✅ Fix first sentence
                : `<li>${advice.replace(/^(\d+\.\s*)/, "")}</li>` // ✅ Remove duplicate numbering
        ).join("")
