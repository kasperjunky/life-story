// Version 3.7 Final - Removing Bold Formatting from Practical Advice and Refining Insight Agent 1 & 2
...
async function showResults() {
  ...
  setTimeout(async () => {
    ...
    if (rewriteInsights?.encouragingRewrite) {
      ...
      // Remove unwanted introductory phrases and fix pronoun issues
...
adviceButton.addEventListener("click", async () => {
  ...
  if (insights?.practicalAdvice && insights.practicalAdvice.length > 0) {
    ...
    const formattedAdvice = adviceList.map((advice, index) => {
      // For all advice, format as a list item without bold markers
