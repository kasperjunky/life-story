// Version 1.8 - Fixing "Your Life Story" Perspective & Refinements
    ...
    try {
        ...
        const response = await fetch("https://interactive-backend.onrender.com/api/generate", {
            ...
            body: JSON.stringify({ 
                ...
                includeAdvice,
                prompts: {
                    storySummary: "Analyze the user's responses and generate a narrative summary about their life. The response **must use second-person perspective ('you')** instead of third-person ('the user'). Start with: 'Based on the answers provided, your life story can...' and ensure a direct, engaging tone.",
                    encouragingRewrite: "Rewrite the user's life story in a way that highlights resilience, personal growth, and positive reframing. Acknowledge struggles while offering an encouraging and realistic perspective.",
    ...
    try {
        ...
        if (insights.storySummary) {
            storySummary.textContent = insights.storySummary
                .replace(/the user|this user|they/g, "you") // Ensures second-person
                .replace(/^Based on the questionnaire answers provided,/, "Based on the answers provided,"); // Fixes phrasing
        }
