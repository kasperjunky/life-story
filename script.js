    ...
    try {
        ...
        const response = await fetch("https://interactive-backend.onrender.com/api/generate", {
            ...
            body: JSON.stringify({
                ...
                includeAdvice,
                prompts: {
                    storySummary: "Summarize the user's answers objectively without any added encouragement. Use only second-person ('you', 'your') without phrases like 'questionnaire answers provided' or 'this user'. It must begin exactly with: 'Based on your answers, you have' followed by a clear, factual summary.",
...
async function showResults() {
    ...
    if (insights?.storySummary) {
        ...
        const prefix = "Based on your answers, you have ";
        // Remove duplicate prefix if it exists at the start
...
adviceButton.addEventListener("click", async () => {
    ...
    if (insights?.practicalAdvice && insights.practicalAdvice.length > 0) {
        // Limit to a maximum of 5 and ensure at least 3 items if available
        const adviceList = insights.practicalAdvice.slice(0, 5);
        const formattedAdvice = adviceList.map((advice, index) => {
            if (index === 0) {
                // Ensure first advice is personal and not preceded by a bullet
                return `<strong>${advice.replace(/This individual has/gi, "You have").replace(/^[•\d\.\s]*/, "").trim()}</strong>`;
            } else {
                // Format subsequent advice as list items without duplicate numbering/bullets
